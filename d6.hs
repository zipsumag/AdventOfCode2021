...
updateSchool (x:xs) = (take 6 rotated) ++ [x + (rotated !! 6)] ++ (drop 7 rotated)
  where rotated = xs ++ [x]

count :: Eq a => a -> [a] -> Int
count n = length . filter (==n)

-- For the naive approach (doesn't work with 256 days)
updateFish :: Int -> Int
updateFish n
  | n > 6     = n - 1
  | otherwise = (n - 1) `mod` 7

updateAllFish :: [Int] -> [Int]
updateAllFish x = (replicate (count 0 x) 8) ++ (map updateFish x)

main :: IO ()
main = do
  --print . length . head . drop 80 . iterate updateAllFish $ fish
  print . sum . head . drop 80 . iterate updateSchool $ tickCounts
  print . sum . head . drop 256 . iterate updateSchool $ tickCounts
  where fish       = map (read :: String -> Int) . separateBy ',' $ input
        tickCounts = [count i fish | i <- [0..8]]